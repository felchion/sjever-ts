...
void create()
{
	set_name (HIY"贵宾年卡"NOR, ({ "vy card" , "card" }));
	set("long","这是一张"HIG"书剑贵宾卡"NOR"，使用(use)后可以增加三百六十五天贵宾时间。\n");
...
int do_use(string arg)
{
  ...
  if (!restrict()) {return notify_fail("本周你已经不能使用"+this_object()->query("name")+"了。\n");}   
	...
	if (!(int)me->query("vip/vip_start_time"))
	{
		...
		tell_object(me,"你使用"HIG"书剑贵宾卡"NOR"开通了三百六十五天的贵宾，请使用 time 命令进行查看。\n");
	}
	else
	{		
		me->add("vip/vip_time",vip_time);			
		tell_object(me,"你使用"HIG"书剑贵宾卡"NOR"增加了三百六十五天的贵宾，请使用 time 命令进行查看。\n");
	}
	if (me->query("vip/vip_total")<0)
		me->set("vip/vip_total",vip_time);
	else
		me->add("vip/vip_total",vip_time);
	log_file("static/VIP",sprintf("| %s(%s)使用VIP卡成功增加了三百六十五天贵宾时间，过期时间(%d)。\n",me->query("name"),me->query("id"),me->query("vip/vip_time")));
	degree();
	return 1;
}	 