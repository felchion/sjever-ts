...
void start_mapping(int max_node)
{
    ...
    start = find_object("/d/city/kedian");
    if (! objectp(start)) {
	start = load_object("/d/city/kedian");
    }
    if (! objectp(start)) {
	STATUS("can't find kedian???\n");
    ...
    STATUS("start mapping with max_node %d\n", max_node);
    mapit(max_node);
}
...
int mapit(int max_node)
{
    ...
    // kill old mapdb
    while (objectp(mapdb=find_object("/clone/obj/mapdb"))) {
	destruct(mapdb);
    }  
