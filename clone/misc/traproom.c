...
void create()
{
        ...
        set("long", HIR @LONG
        这是一口不知是谁挖掘的陷阱。陷阱深达数丈，由下往
上望去，只能见到桌面般大的一片天空。陷阱周围是松软的土层，
不停地往外渗水。似乎刚刚挖好没多久。
LONG NOR
);      
...
void init()
{
        ...
        if( me->query("rided") ) me->delete("rided");
...
int do_jump(string arg)
{
        ...
        if( depth > power && depth / 2 < power && me->query("jingli") >= depth/10 ) {
                me->receive_damage("jingli", depth/10, "在陷阱里累死了");
...
int do_climb(string arg)
{
        ...
        if( depth /2 > power && depth / 3 < power && me->query("jingli") >= depth/5 ) {
                me->receive_damage("jingli", depth/5, "爬陷阱累死了");
...
int do_push(string arg)
{
        ...
        if( depth > power_me || me->query("jingli") < depth/5 )
...
//      if( depth / 3 < power_ob )
//      return notify_fail("陷阱并不深，对方可以自己想办法出去，用不着你帮忙。\n");

        me->receive_damage("jingli", depth/5, "救人出陷阱时累死了");
...
int do_fill()
{
        ...
        if( (depth*2 > power_me || me->query("jingli") < depth/3) 
        &&  me != roomtrap->query("owner") )
        return notify_fail("这个陷阱太深了，想填住它，非累死不可。\n");

        me->receive_damage("jingli", depth/3, "填陷阱时累死了");
...
//              tell_object(owner, HIY "I am called again !\n" NOR);
                if( environment(owner) == roomtrap || environment(owner) == roomfrom || environment(owner) == roomto )
                        count = 0;
                else    count++;
                call_out("maintaining", 120, roomtrap, count);
//              tell_object(owner, HIY "count = " + count + "\n" NOR);
        } else {
                roomfrom->set("exits/" + roomtrap->query("from"), file_name(roomto));           
                roomto->set("exits/" + roomtrap->query("to"), file_name(roomfrom));     
                roomfrom->delete("exits/jump" + roomtrap->query("from"));
                roomto->delete("exits/jump"  + roomtrap->query("to"));
//              tell_object(owner, HIY "destructing ... \n" NOR);
                destruct(roomtrap);
        }
}
