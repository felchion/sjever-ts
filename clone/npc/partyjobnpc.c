...
void create()
{
        ...
        setup();
        if( random(2) ) add_money("silver", random(100) + 1 );
...
void do_copy(object ob)
{
        ...
        set_temp("apply/armor",  random(150));
...
void die()
{
        ...
        if( ob->query("kill_party") == me->query("family/family_name") ) {
                ...
                if( ob->query("combat_exp") > me->query("combat_exp")*3/2 )
                        ...
                        exp = 400 + random(200) + (ob->query("combat_exp") - me->query("combat_exp"))/2000;
                        if( exp >= 1000 ) exp = 800 + random(200);
                } else if( ob->query("combat_exp") < me->query("combat_exp")*3/4 )
                        exp = 300 + random(300);
                else exp = 400 + random(200);
                ...
                message_vision(YEL"\n$N杀死了" + ob->name() + "，为" + me->query("family/family_name") + "清除了来犯的恶贼。\n",me);
                message_vision(YEL"$N仔细回忆刚才和" + ob->name() + "的打斗经过，发现自己对武学的领悟又更深了一层！\n"NOR,me);
                tell_object(me, HIW"你获得了" + CHINESE_D->chinese_number(exp) + HIW"点经验，" +
...
void kill_ob(object me)
{
        ...
        if( me->query("family/family_name") == ob->query("kill_party") )
                ::kill_ob(me);
        else {
                me->remove_killer(ob);
                remove_killer(me);
