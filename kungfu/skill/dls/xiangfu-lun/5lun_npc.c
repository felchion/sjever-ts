...
{
	int lvl=me->query_skill("longxiang-boruo", 1);
	set("name", me->name());
	...
	set_temp("apply/damage", 500+lvl);
	set_temp("apply/pfm_damage", 200+lvl/3);
	set_temp("apply/pfm_attack", 300+lvl/10);
	weapon->wield();
}
...
void heart_beat()
{
	...
	if( environment(target) == environment(me) ){
		...
		if( me->is_fighting(target) && !random(3) )
