...
int perform(object me, object target)
{
	...
	lv = me->query_skill("xuantie-jianfa", 1);
	...
	if( me->query("env/玄铁剑法") != "海潮" ) {
		...
		me->add_temp("apply/damage", 400+lv);
	...
	&& (int)me->query("neili", 1) > (me->query_skill("force")+ lv)
	&& objectp(target)
	...
	&& me->query("env/玄铁剑法") != "汹涌" ) {
		...
		if( COMBAT_D->do_attack(me, target,0,0,1)) {
			...
			if( me->query("env/玄铁剑法") == "海潮")
				target->add_busy(5+lv/200);
			else
				target->add_busy(2+lv/400);
...
int perform2(object me,object target)
{
	...
	if( COMBAT_D->do_attack(me, target,0,0,1) ){
		...
		message_vision(msg, me, target);
		target->add_temp("apply/pfm_attack", -150);
		target->add_temp("apply/pfm_dodge", -150);
		target->add_temp("apply/pfm_parry", -150);
		target->set_temp("xtjf/haichao", 1);
		if( random(2)) {
			target->add_condition("no_exert", 2+ random(2));
			target->set_temp("xtjf/exert", 1);
			tell_object(me,MAG"由于"RED+name+MAG"被点中，"+target->name()+"暂时无法施用内功。\n"NOR);
		}
		else {
			target->add_condition("no_perform", 2+random(2));
