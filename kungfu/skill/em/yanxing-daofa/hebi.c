...
int perform(object me, string arg)
{
  ...
  if( skill < 300 || skill1 < 300)
    return notify_fail("你们的雁行刀法等级不够，不能使用「双刀合璧」。\n");
  
  if (ABS(skill - skill1)>50)  
    return notify_fail("你们修为相差太远，不能使用「双刀合璧」。\n");
    
  if(me->query_skill("linji-zhuang", 1) < 300 )
    return notify_fail("你的临济十二庄等级不够，不能使用「双刀合璧」。\n");       
  ...
  if(target->query("max_neili") < 5000 )
  ...
  if (target->query_temp("hebi_target") ==me){
    ...
    me->add_temp("apply/pfm_dodge", 100);
    me->add_temp("apply/pfm_parry", 100);
    target->add_temp("apply/pfm_damage", 100);
    target->add_temp("apply/pfm_attack", 100);
    target->add_temp("apply/pfm_dodge", 100);
    target->add_temp("apply/pfm_parry", 100);
    call_out("check_fight", 1, me, 100, (skill+skill1)/20);
    call_out("check_fight", 1, target, 100, (skill+skill1)/20);
    return 1;
  }
