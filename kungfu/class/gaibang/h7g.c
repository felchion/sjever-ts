...
void create()
{
	...
	set_skill("force", 500);
	set_skill("huntian-qigong", 510);    // 混天气功
	set_skill("strike", 500);            // 基本掌法
	set_skill("xianglong-zhang", 530);   // 降龙十八掌
	set_skill("literate", 580);           // 读写
	set_skill("dodge", 500);             // 基本躲闪
	set_skill("xiaoyaoyou", 510);        // 逍遥游
	set_skill("parry", 500);             // 基本招架
	set_skill("whip", 500); 
	set_skill("hand", 500); 
	set_skill("blade", 500); 
	set_skill("staff", 500); 
	set_skill("stick", 500);             // 基本棍杖
	set_skill("dagou-bang", 510);        // 打狗棒法
	set_skill("begging", 300);           // 叫化绝活
	set_skill("checking", 300);          // 道听途说
	set_skill("bangjue", 300);            // 棒决
	set_skill("lianhua-zhang", 520);      // 莲花掌
	set_skill("liuhe-daofa", 510); 
	set_skill("suohou-shou", 520); 
	set_skill("fengmo-zhang", 510); 
	set_skill("huifeng-bian", 500); 
	set_skill("stealing", 300);           // hehe
...
void attempt_apprentice(object ob)
{
	...
	if((int)ob->query_skill("huntian-qigong", 1) < 150) {
...
int ask_up()
{
	...
	if ( (me->query("job_time/丐帮") + me->query("job_time/抓蛇")) < 1000){
...
// 以后去除
	...
	if (me->is_die_dummy()) {
    tell_object(me, HIW"洪七公指点了你一些江湖经验，你获得了一点江湖声望。\n"NOR);
    me->add("honor", 1);
  }
  else {
...
int accept_object(object me, object obj)
{
	...
	if( ob_yzz = present("yuzhu bang", ob) ){
		...
		if ( me->query_skill("xianglong-zhang", 1) < 270 ){
