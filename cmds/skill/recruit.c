// recruit.c
// Modify by Lklv 2002.7.17 今天身体不舒服
		...
		if((ob->query("family")) && ( (string)me->query("family/family_name") != (string)ob->query("family/family_name") ) )
		{
			...
			if (ob->query("combat_exp") > 10000) {
				...
				log_file("static/BETRAY",
					ob->query("name")+"("+ob->query("id")+")第"+count+"次从"
					+ob->query("family/family_name")+"叛入"+me->query("family/family_name")+"\n",
					ob
				);
				if (count <= 1)
					count = 10;
