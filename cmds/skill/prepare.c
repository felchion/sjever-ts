...
int main(object me, string arg)
{
	...
	if (sscanf(arg, "%s %s", arg1, arg2) == 2)
	{
		...
		return notify_fail("你如何能一心二用。\n");

		if ( SKILL_D(arg1)->query_class() == "空"
		|| SKILL_D(arg2)->query_class() == "空"
		|| !SKILL_D(arg1)->query_class() 
		|| !SKILL_D(arg2)->query_class()  )
		...
		|| SKILL_D(arg1)->query_class() == "空"
		|| SKILL_D(pmap[skill[0]])->query_class() == "空"
		|| !SKILL_D(pmap[skill[0]])->query_class()
		|| !SKILL_D(arg1)->query_class() ) )
		return notify_fail(to_chinese(arg1)+"不能用于你现有的拳脚技能组合。\n");
		...
		if (flag) {
			if (me->query_temp("perform_AP") < 40)
			  return notify_fail("你合气度不足无法准备技能。\n");
		  else 
