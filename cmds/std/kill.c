...
//	if (function_exists("prevent_learn", obj) && obj->query_skill("force",1)<400) 
//	return notify_fail("你不能对"+obj->name(1)+"下手。\n");
  ...
  && me->is_dummy() 
  && !obj->is_killing(me->query("id")) 
  && !me->query_temp("revenge/"+obj->query("id")) )
  return notify_fail("你上周在线或成长不足，无法攻击对方。\n");
	...
	if (me->is_killing(obj->query("id")) && me->is_fighting(obj)) {
		...
		if (userp(enemy[i])) return notify_fail("加油！加油！\n");
		if(time() - me->query_temp("last_ap_give")>60) {
			me->add_temp("perform_AP",me->query("int")*3+me->query_temp("apply/ap"));
			me->set_temp("last_ap_give",time());
			return notify_fail(HIY"加油！加油！\n");
		}
		else return notify_fail("加油！加油！\n");
	}
	...
	if (!userp(obj)) {
	  if (!me->is_fighting()) {
	  	...
	  	for(i=0;i<sizeof(enemy);i++)
		    if (userp(enemy[i])) flag = 1;
		  if(!flag && (time() - me->query_temp("last_ap_give")>60) ) {
		    me->add_temp("perform_AP",me->query("int")*3+me->query_temp("apply/ap"));
		    me->set_temp("last_ap_give",time());  
		  }	
	  }
