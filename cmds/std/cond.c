...
//		case "bonze_jail"	: name = "少林监狱"; break;
		...
		case "task_busy"		: name = "任务繁忙状态"; break;
		case "db_exp"		: name = "双倍经验"; break;
		case "tp_exp"		: name = "明悟"; break;
		case "true_view"		: name = "真实视野"; break;
		case "ggss"		: name = "鬼谷算术"; break;
		case "inv_keep"		: name = "聚宝"; break;
...
├────────────────────────┤\n";
		...
		for ( i=0; i<sizeof(cons); i++ )
		{
			...
			if(!conob->is_fast()) {
				if (values(cons)[i]["cnt"] < 3)
					cont = chinese_number(values(cons)[i]["cnt"]*20)+"秒";
				else
					cont = chinese_number(values(cons)[i]["cnt"]/3)+"分";
			}
			else cont = chinese_number(values(cons)[i]["cnt"]*2)+"秒"; 	
			...
			if(!stringp(conn=conob->query_name())) conn = get_con_name(coni); 	
			if (conn == "不详")	continue;
