...
int main(object me, string arg)
{
    ...
    if (dbquery("DELETE FROM Data WHERE U_ID='" 
		...
		if ( (dbquery("update members set extcredits1 =-999999999 WHERE username = '"+ name +"@"+lower_case(INTERMUD_MUD_NAME)+"'"))
		&& (dbquery("update members set credits = -999999999,username = CONCAT('删档用户',uid),groupid=21  WHERE username = '"+ name +"@"+lower_case(INTERMUD_MUD_NAME)+"'")))					
				tell_object(me,HIY"用户BBS帐号已经成功删除！\n"NOR);	
		if (dbquery("update discuz6.uc_members set username = CONCAT('删档用户',uid) WHERE username = '"+ name +"@"+lower_case(INTERMUD_MUD_NAME)+"'"))
				tell_object(me,HIY"用户ucenter帐号已经成功删除！\n"NOR);	;
