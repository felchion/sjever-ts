...
int main(object me, string arg)
{
	...
	int i_all=0,i_feihu=0,i_tlbb=0,i_lchj=0;
	...
	{     
		...
		default :
			return notify_fail("目前只开放雪山飞狐、天龙八部和连城诀。详细内容请查阅帮助。\n");
		}
		if (wizardp(me))
		{
	...
	if(i_all==1)  
	{
		...
		str = quest_lchj(ob);
	else return notify_fail("目前只开放雪山飞狐、天龙八部和连城诀。详细内容请查阅帮助。\n");
...
string quest_tlbb(object me)
{
    ...
    if(!me->query("quest/天龙八部/凌波微步篇/over"))
	{
		if(!me->query("quest/天龙八部/凌波微步篇/wuliangover"))
		{
			...
			else str +=get_string(HBRED"寻找失踪的段誉中，注意别忘记随便解密凌波微步。"NOR);
		}
		else
		{
