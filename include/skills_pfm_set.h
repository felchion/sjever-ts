...
//							u_w_pfm=u_w_pfm + ({(: perform_action, "sword.hebi" :)});       //冲灵剑法pfm：合壁
				...
				case 1:
					...
					switch(random(2))
					{
						case 0:
							...
							u_w_pfm = u_w_pfm + ({(: perform_action, "sword.feijian" :)});     	//华山剑法pfm：飞剑
							u_w_pfm = u_w_pfm + ({(: perform_action, "sword.junzi" :)});		//君子
							u_w_pfm = u_w_pfm + ({(: perform_action, "sword.kuaijian" :)});		//快剑
							u_w_pfm = u_w_pfm + ({(: perform_action, "sword.lianhuan" :)});		//连环
							u_w_pfm = u_w_pfm + ({(: perform_action, "sword.sanqingfeng" :)});	//三清风
						...
						case 1:
							...
							u_w_pfm = u_w_pfm + ({(: perform_action, "sword.po" :)});	
		...
		case "tz":												//设定铁掌武功
			...
			u_f_pfm=u_f_pfm + ({(: exert_function, "juli" :)});          //铁掌气功pfm：聚力
				...
				case 1:
					...
					switch(random(1))
					{
						case 0:
							...
							u_w_pfm = u_w_pfm + ({(: perform_action, "sword.mie" :)});				//灭
							u_weapon="sword";
