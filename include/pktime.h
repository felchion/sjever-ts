  ...
  int hour;
	...
	if( me!=target &&  interactive(me) && (!sizeof(ids) || member_array(getuid(me),ids) == -1 ) )//不在攻击过的列表内才采取措施 再加上必须是连线的玩家
	{
		...
		if(ips && sizeof(ips) && member_array( query_ip_number(me),ips)!=-1 && check_idip && sizeof(check_idip)>0 ) //得出ML PK的结论
		{
			...
			tell_object(me,BLINK HIR"你的犯规行为已经被系统记录，请立刻到百姓留言版自首你的犯规行为。\n\n"NOR NOR);
		}
	...
	sscanf( ctime(time()), "%s %*s  %*d %d:%*s",ts,hour);
	...
	if ( ts == "星期日") {
		if(hour > 7) 
		  ...
		  notify_fail("今天是"+ts+"，您只能在八时以后主动攻击行为。\n");
		  return 1;
		}		  
	}
	else {
      ...
      return 0;
    	...
    	notify_fail("今天是"+ts+"，您只能在十八时以后主动攻击行为。\n");
	    return 1;
	  }
  }
}
