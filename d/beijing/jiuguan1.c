//  /u/jpei/beijing/jiuguan1.c

#define KICK_TO	__DIR__"road1"
...
void create()
{
	set("short", "小酒馆");
	set("long", @LONG
这是北京西城的一家小酒店。你一走进来，就闻到一股酒菜的香味，勾人
食涎。厅内摆着几张八仙桌(table)， 桌上整齐地摆放着碗筷，四周放着些椅
子(chair)。 一个酒保正在忙个不停。你要想打听江湖掌故和谣言，这里是个
好所在。柜台上立着一个价目表(sign)。
LONG
	);
	...
	set("objects",([
		__DIR__"npc/zhanggui.c" : 1,
		__DIR__"npc/jiubao.c" : 1,
	]) ); 
...
int do_order(string arg)
{
	...
	if (me->is_busy() || me->is_fighting())	return notify_fail("你正忙着呢！\n");
	if (!me->query_temp("marks/sit")) return notify_fail("酒保用好象瞧见一个怪物一样的眼神打量着你：“站着吃喝？有位子不坐，您不嫌累呀？”\n");
	if (!arg) return notify_fail("酒保不耐烦地说：“你究竟要什么啊？”\n");
			
	serve = query_temp("serve");
	if (serve == "无") return notify_fail("酒保赔笑道：“客官见谅，厨房已经停火了！”\n");
