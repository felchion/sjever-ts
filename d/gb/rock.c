...
int do_strike(string arg)
{
        ...
        if ( me->query("max_neili") <= 300 ) {
                ...
                message_vision("$N大喝一声，往岩上一击，只听见" +
                        "‘嗡...’，岩石摇了几下。\n",me);
        if ( me->query_skill("strike", 1) <= 121 ) {
                skl_impv = (me->query_str() / 2) + 
                        (me->query_con() / 2) +
                        (me->query_condition("drunk") / 2) ; 
                me->improve_skill("strike", skl_impv);
                
                jingli_damage = 80 - random(me->query_str()) -
                        random(me->query_int()) -
                        random(me->query_con());
