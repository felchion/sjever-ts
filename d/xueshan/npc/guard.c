...
string *title1 = ({ "京城", "扬州", "西南", "东北", "中原","江南","江北","西南","中原","西北","河南","河北","东北"});
string *title2 = ({ "镖头", "大侠", "捕头", "护院", "保镖"});
...
void create()
{
	...
	set_name(name["name"], name["id"] + ({ "guard" }));
...
void die()
{
	...
	if(objectp(me->query("beauty")))  (me->query("beauty"))->set("ok",1);
	::die();
}
...
int do_copy(object target,int mode)
{
	...
	me->set("max_qi", (int)ob->query("max_qi")*(mode?2:1));
	me->set("eff_qi", (int)ob->query("max_qi")*(mode?2:1));
	me->set("qi", (int)ob->query("max_qi")*(mode?2:1));
	me->set("max_jing",(int)ob->query("max_jing")*(mode?2:1));
	me->set("eff_jing",(int)ob->query("max_jing")*(mode?2:1));
	me->set("jing",(int)ob->query("max_jing")*(mode?2:1));
	me->set("max_neili",(int)ob->query("max_neili")*(mode?2:1) );
	me->set("neili",(int)ob->query("neili")*(mode?2:1));
	if( me->query("neili") < me->query("max_neili") )
		me->set("neili",(int)me->query("max_neili") );
	me->set("max_jingli",(int)ob->query("max_jingli")*(mode?2:1));
	me->set("eff_jingli",(int)ob->query("eff_jingli")*(mode?2:1));
	me->set("jingli",(int)ob->query("jingli")*(mode?2:1));
...
string ask_leave()
{
	...
	if (!objectp (ob->query("xs/girl_ob")) || (ob->query("xs/girl_ob") != me->query("beauty")))
		return "今日有公务在身，恕不奉陪。";
...
void kill_ob(object ob)
{
	...
	command("!!!");
	if( !objectp (ob->query("xs/girl_ob"))
	 || (ob->query("xs/girl_ob") != me->query("beauty"))) {
		command("say 快逃啊！");
