// Room: /d/plummanor/entrance.c 庄院大门
// By Jpei
...
LONG
	...
	set("item_desc", ([
		"gate" : "大门紧闭着，门上有个擦得精光雪亮的大铜环，看来是敲(knock)门用的。\n",
	]) );
	set("exits", ([
		"north" : __DIR__"road3",
	]));
...
int do_knock(string arg)
{
	...
	if (sscanf(arg, "%s %d %s", gate, count, times) != 3 || gate != "gate" || times != "times")
		return notify_fail("你要敲几次(knock gate ? times)？\n");
...
int do_answer(string arg)
{
	...
	if (arg[0..5] == "求见江南四友" || arg[0..5] == "拜见江南四友" ||
		arg[0..7] == "有事求见江南四友" || arg[0..7] == "有事拜见江南四友") {
		if (!present("shi lingwei", env))
			new(__DIR__"npc/shilingwei")->move(env);
		if (!present("ding jian", env))
			new(__DIR__"npc/dingjian")->move(env);
		write("你说道：“有事求见江南四友(four-friends)，四位前辈。”\n");
		tell_room(env, me->name() + "对着两个家人说了些求见的话。\n", ({me}));
