...
void create()
{
        set("short", "大渡口");
        set("long", @LONG
这里是兰州黄河大渡口，是通往西域的必经之路。宽阔的黄河江面上飘荡
着几只轻帆。渡口处正停着一艘渡船，几个乘客正在和船家讨论价钱。你可以
由此上(yell)船渡过黄河。
LONG);
	set("exits", ([
		"east" : __DIR__"road6",
...
int du_jiang()
{
	...
	tell_room(environment(me), HIC+me->name()+"长袖飘飘，从河面上飞跃过来，在船上一点，又向西跃出！\n" NOR, ({me}));
        write("你在江中渡船上轻轻一点，又提气飞纵向西岸。\n");
        me->move(SHIP_TO);
