...
void die() 
{
	...
	if (!me->query("quest/jiuyin2/start") && !me->query("quest/jiuyin2/pass")) {
		::die();
		return;
	}
	
	if( (random(1000)+1 > (5*(100+(me->query("int")-20)*3))) 
	|| me->query("quest/jiuyin2/pass") ){
		if(! me->query("quest/jiuyin2/pass")) {
			log_file("quest/jiuyin",sprintf("%-18s从陈玄风处得到九阴真经下卷。\n",
              			me->name(1)+"("+capitalize(getuid(me))+")"), me);
		  me->delete("quest/jiuyin2/start");
