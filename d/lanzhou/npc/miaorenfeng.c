...
int thinking(object me, object ob)
{
  ...
  if(me->query_temp(QUESTDIR4+"thinking")<(3+random(3)))
         ...
         if(p > 37&& me->query("kar")<31)
    {
	   ...
	   log_file("quest/FEIHU", sprintf(HIR"%s(%s) 失败%d次后揭开苗家剑法,获得%d点声望。random（30）取值%d。\n"NOR, me->name(1),me->query("id"),me->query(QUESTDIR4+"miaojiajianfa_fail"),i,p) );
    }
    else
    {
...
int ask_hujiadaofa() 
{
  ...
  if(me->query(QUESTDIR4+"hujiadaofa") && me->query(QUESTDIR4+"miaojia-jianfa") )
  {
  	  //非vip玩家，只能失败三次
