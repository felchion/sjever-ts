...
string look_left(object me)
{
	return YEL"这张图谱是一个卷轴舒开，帛面年深日久，已成焦黄之色，图上开头写着： \n"+
		  ...
		  "着内力的运走径道，旁边注着一行小字“少冲剑运气法门”。\n" NOR;
}
string look_right(object me)
{
	return YEL"这张图谱是一个卷轴舒开，帛面年深日久，已成焦黄之色，图上都是纵横交叉\n"+
		  "的直线、圆圈和弧形，但见红线黑线，纵横交错，头绪纷繁之极，这图便是少\n"+
		  "冲剑的剑法，招式并不见复杂，却是快捷无比。\n" NOR;
...
int do_study(string arg)
{
	...
	if(arg=="left" ) {
		...
		if (neili < (j+1) * 3000 + 1500 && !me->query("liumai-shenjian/shaochong"))
			return notify_fail("你竭尽全力的驱动体内的一阳指真气，试图由少冲穴射出无形剑气，却无半点劲气发出。\n");
		if (lvl > 30 * (j+1))
		...
		if (lvl>=30*(j+1)||me->query("liumai-shenjian/shaochong",1)){
			write("你内力运转自如，凝聚真气，小拇指捺出，嗤嗤的一声，一条无形剑气自“少冲”穴中激射而出。\n");
			...
			write("你照着图谱研习少冲剑法的运气法门，渐渐的悟出其中的精要所在。\n");
			if (!me->query("liumai-shenjian/shaochong"))
				me->improve_skill("liumai-shenjian", me->query("int"));
		}
		return 1;
