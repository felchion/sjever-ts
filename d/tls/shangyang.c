...
string look_left(object me)
{
	return YEL"这张图谱是一个卷轴舒开，帛面年深日久，已成焦黄之色，图上开头写着： \n"+
		  ...
		  "剑气。。紧跟着帛上绘着个裸体男子的图形，身上注明穴位，以红线黑线绘着\n"+
		  "内力的运走径道，旁边注着一行小字“商阳剑运气法门”。\n" NOR;
}

string look_right(object me)
{
	return YEL"这张图谱是一个卷轴舒开，帛面年深日久，已成焦黄之色，图上都是纵横交叉\n"+
		  "的直线、圆圈和弧形，但见红线黑线，纵横交错，头绪纷繁之极，这图便是商\n"+
		  "阳剑的剑法，巧妙活泼，难以捉摸，变化极其复杂。\n" NOR;
}
...
int do_study(string arg)
{
	...
	if(arg=="left" ) {
		...
		if ((j+1) > 2 && me->query_skill("liumai-shenjian", 1) >= 60)
			return notify_fail("你已经竭尽全力去体会图谱上的精要，可是总感觉体内数道内劲无法运转自如！\n"); 
		if (neili<(j+1)*3000+1500&&!me->query("liumai-shenjian/shangyang"))
			return notify_fail("你竭尽全力的驱动体内的一阳指真气，试图由商阳穴射出无形剑气，却无半点劲气发出。\n");
		if (lvl>30*(j+1))
		...
		if (lvl>=30*(j+1)
		 ||me->query("liumai-shenjian/shangyang",1)){
			write("你内力运转自如，凝聚真气，食指点出，嗤嗤声响，一条无形剑气自“商阳”穴中激射而出。\n");
			...
			write("你照着图谱研习商阳剑法的运气法门，渐渐的悟出其中的精要所在。\n");
			if (!me->query("liumai-shenjian/shangyang"))
				me->improve_skill("liumai-shenjian", me->query("int"));
		}
		return 1;
