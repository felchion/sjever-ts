...
然便是半个骷髅骨头。他在天龙寺中辈份最高，
这里独参枯禅已数十年，天龙寺诸僧众，谁也没也
没过他真面目。\n");
	set("gender", "男性" );
	set("class","bonze");
	set("unique", 1);
	...
	set_skill("whip", 480);
	set_skill("axe", 480);
	set_skill("cihang-bian", 460);
	set_skill("duanjia-jianfa", 470);
	set_skill("sword", 470);
	set_skill("strike", 460);
	set_skill("buddhism", 200);
	set_skill("literate", 530);
	set_skill("qiantian-yiyang", 460);
	set_skill("liumai-shenjian", 240);
	set_skill("kurong-changong", 460);
	set_skill("qingyan-zhang", 480);
	set_skill("yiyang-zhi", 460);
	set_skill("tianlong-xiang", 470);
	set_skill("pangen-fu", 460);
...
string ask_abandon()
{
	...
	if (myfam["family_name"] != "天龙寺"||!me->query("tls")||me->query("class") != "bonze"){
		return "对不起，你在说什么？\n";
	}
	
	if (me->query_skill("liumai-shenjian",1)<30 
	|| !mapp(me->query("liumai-shenjian")) 
	|| me->query("max_neili")<1000)
		  return "大师何出此言？";
  me->delete("liumai-shenjian");
  me->set_skill("liumai-shenjian",0);
  me->add("max_neili",-1000);   
  message_vision("枯荣禅师伸出右手按在$N的顶门，一股奇异的真气传遍$N的全身。\n",me);
  me->start_busy(10);
  return "善哉，善载。";
}  
string askjiumozhi()
