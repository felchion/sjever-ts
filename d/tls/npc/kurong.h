...
string ask_liumai2()
{
	...
	if (ob->query("liumai_pass") && (ob->query_skill("liumai-shenjian",1) < 61)) {
	...
	if (ob->query_skill("liumai-shenjian",1) > 60){
		ob->set("quest/天龙八部/武功/liumai_update",1);
		return "你的六脉神剑修为已经登峰造级，老衲这里也不能给你什么帮助了，\n"+
			"你去找镇南王世子段誉吧，也许他能给你些帮助。";
	}
	else {
		command("say 依照天龙寺祖上传下规矩，" + RANK_D->query_respect(ob) + "你必须接下我三招，才够资格进普云洞修习六脉神剑。");
		ob->set_temp("try",1);
		me->set_temp("try",1);
