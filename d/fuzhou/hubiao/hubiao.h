...
//        	return 1;
//        }
	if (sizeof(filter_array(children(__DIR__"obj/cart"), (: clonep :))) > 9) {
	...
	for (i=0;i<sizeof(team);i++){
		...
		if ( team[i]->query("ll/last_count") < 15 )
		{
			command("say 现在暂时没有镖要护送。");
			return 1;
		}
...
//	endname = get_place(base_name(place))+place->query("short");
	...
	message_vision("$N将护镖押金"+chinese_number(sizeof(team)*20)+"两黄金交给了镖局。\n", me);
