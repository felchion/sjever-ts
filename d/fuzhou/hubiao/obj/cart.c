...
//	me->start_busy(random(3) + 2);
	...
	if (!newroom->query("no_fight") && !query_temp("s_time") && random(10) < 6) {
		...
		tell_room(newroom,HIR"突然跳出个劫匪！\n"NOR);
		tell_room(newroom,HIR+jf->query("name")+"恶狠狠地说道：乖孙子，留下钱财好好孝敬你爷爷吧。\n"NOR);
/*
		for(i=0; i<sizeof(obj); i++){
	...
	if (j > 0) {
		...
		for (i=0; i < sizeof(team); i++)
			if (team[i]->query_temp("protecting")==cart) {
				...
				tell_object(team[i],HIY"你被奖励了"+chinese_number(jl)+"点经验，"+chinese_number(j)+"点潜能，"+chinese_number(30)+"两黄金！\n"NOR);
				log_file("job/hubiao", sprintf("%8s%-10s护镖奖励经验%4d,潜能%3d,现在经验%d,应得%d,步数%d,费时%d,劫匪%d,推时%d/%d,走错%d。\n",
					team[i]->query("name"),
	...
	if( j >= sizeof(team)){
		for(j=0; j<sizeof(team); j++) {
			...
			if (team[j]->query_temp("protecting")==cart)
				...
				team[j]->apply_condition("job_busy", 6); 
			tell_object(team[j],HIR"由于你护镖失败，交给镖局的二十两黄金被用来赔偿了。\n"NOR);
		}
...
int checkteam()
{
	...
	for(j=0; j<sizeof(team2); j++){
		...
		if (team2[j]->query_temp("protecting")==cart)
			...
			team2[j]->apply_condition("job_busy", 6); 
		tell_object(team2[j],HIR"由于你护镖失败，交给镖局的二十两黄金被用来赔偿了。\n"NOR);
	}
