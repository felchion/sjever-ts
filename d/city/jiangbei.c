...
int du_jiang()
{
	...
	if (me->query("neili") < 1000 || me->query("jingli") < 1200){
        ...
        && userp(room) && !wizardp(room)) {
                tell_room(environment(me), YEL+me->name()+YEL+"长袖飘飘，从江面上飞跃过来，在"+room->name()+YEL+"头上一点，又向南跃出！\n" NOR, ({me, room}));
                tell_object(room, YEL+me->name()+YEL+"长袖飘飘，从江面上飞跃过来，在你头上一点，又向南跃出！\n" NOR);
        } else tell_room(environment(me), YEL+me->name()+YEL+"长袖飘飘，从江面上飞跃过来，在船上一点，又向南跃出！\n" NOR, ({me}));
