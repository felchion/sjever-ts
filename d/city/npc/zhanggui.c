...
string rumor() 
{
        ...
        || me->query("quest/whdmd/start") 
        || me->query("quest/whdmd/pass") ) 
        	return "最近也没听说有什么传闻。";        
        else {
 	   	    me->add("quest/chance",-1);
    		  me->set("quest/whdmd/start",1);
   		    log_file("quest/wuhu",sprintf("%8s%-18s开始解五虎断门刀花费解迷机会一次。\n",
...
string mao18()
{
        ...
        if (me->query_temp("total_pay") && me->query("quest/whdmd/start")) {
                me->set_temp("mao18/jg_pass",1);
                return "对，就是那个人了。听说那个江洋大盗躲到丽春院快活去了。";
        }
        return "你在胡说些什么呀，小心砍头。";
}
