...
void create()
{
	set("short", "饭厅");
	set("long", @LONG
这是桃花岛弟子们每日吃饭的地方，饭菜的香味充溢着整个房间。屋里摆
着一张大桌子(table)和一些椅子(chair)。屋中站着几个哑仆。
LONG
	);
...
int serve_food(object me)
{
	...
	new(__DIR__"obj/mifan.c")->move(me);
...
int do_sit(string arg)
{
	...
	if (local[2] < 6 || (local[2] > 8 && local[2] < 11) || (local[2] > 13 && local[2] < 17) || local[2] > 20) {
