...
int ask_jieyi()
{
	...
	if ( !ob->query("quest/jiebai/start") ) {
		command("shake");
...
//		return 1;
//	}
	if ( !ob->query("quest/jiuyin1/pass") && time < 86400) {
		command("say 我很忙呢。");
		return 1;
	}
	if ( ob->query("quest/jiuyin1/fail") >= i && !ob->query("quest/jiuyin1/pass")) {
		command("say 我真的很忙呢。");
		return 1;
	}
...
int ask_hubo()
{
	...
	if(ob->query("quest/hubo/start")) {
		ob->set_temp("zuoyou_hubo", 1);
		command("say 让你练你就快练嘛！");
		return 1;
	}

	if(ob->query("quest/chance")<1 ){
		tell_object(ob,"周伯通正盯着你看，不知道打些什么主意。\n");
		command("say 估计你现在是学不会了，还是以后再来吧。");
		return 1;
	} 
	...
	ob->add("quest/chance",-1);
	ob->set("quest/hubo/start",1);
	ob->set_temp("zuoyou_hubo", 1);
	log_file("quest/jiebai",
		sprintf("%-18s想从周伯通处领悟左右互搏花费解迷机会一次。\n",
	  	ob->name(1)+"("+capitalize(getuid(ob))+")"
		), ob
	 );
	return 1;
...
int do_fangyuan()
{
	...
	if (!hubo || !ob->query("quest/hubo/start")) return 0;
	if (hubo < 21) {
	...
	if (hubo == 22) {
		...
		if( !random(30) ){
				...
				log_file("quest/jiebai",sprintf("%-18s想从周伯通处领悟双手互搏失败。\n",
					ob->name(1)+"("+capitalize(getuid(ob))+")" ), ob);
				return 1;
			}
	}
	return 0;
