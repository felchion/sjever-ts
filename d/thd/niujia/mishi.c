...
void create()
{
	...
	set("short", "密室");
	set("long", @LONG
这是一间设计精巧的密室，并不很大，但是却不让人觉得拥挤。地下有一
具干枯的骸骨(haigu)，旁边则是一口生锈了的大铁箱(xiang)。墙上有一个小
小的圆孔(kong)，通过它能看到外面的情况。
LONG
	);
	set("no_clean_up", 0);
...
int do_move(string arg)
{
	...
	if (arg == "haigu") {
		...
		&& !present("qulingfeng tiepai",this_object())) {
			...
			set("tiepai_got",1);
			new(__DIR__"obj/tiepai.c")->move(this_object());
...
int do_open(string arg)
{
	...
	if (arg == "xiang") {
		...
		thdx = random(20) + 30;
		thdy = random(20) + 30;
		write("你用劲打开了箱子，发现里面竟藏有着无数的大内密宝。而在珠宝的下面，有一张发黄的海图。");
	...
	if (arg == "jiaceng") {
		if (me->query_temp("thd_jiaceng") == 1) {
			...
			write("你轻轻地翻开夹层，发现里面竟然是一轴轴的画卷(painting)，都是名家所做的精品。\n");
			return 1;
		}
...
int do_browse(string arg)
{
	...
	if (painting_no == me->query_temp("thd_shanshui") && !query("shanshui_got") && me->query_temp("find_shanshui")
	&& !present("shanshui hua", me)	&& !present("shanshui hua", this_object())) {
		write("你随手打开一轴画卷，是一幅泼墨山水，令你觉得很奇怪的是，此画风骨虽佳，但少了含蕴韵致，不象是名家手笔。\n");
		new(__DIR__"obj/shanshui.c")->move(this_object());
		set("shanshui_got",1);
// add for tz
